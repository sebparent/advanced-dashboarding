...
export function buildResultSQL(spec, rows, target = "Metabase") {
  ...
  return {
    ...
    charts: [
      // carry full columns/rows too, so a "table" report shows every column
      { type: spec.chart_type || "bar", title: spec.title, desc: spec.intent, labelKey: label, valueKey: value, data: rows, columns, rows },
