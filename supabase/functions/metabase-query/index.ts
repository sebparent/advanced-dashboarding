...
async function interpret(prompt, tables, extra) {
  ...
  const system = "Tu traduis une demande en francais en une requete SQL de LECTURE SEULE pour un entrepot interroge via Metabase (Postgres).\n" +
    ...
    "- STOCK (niveau de stock) : source = logistic_management.stocks (s) ; stock = SUM(s.quantity) en unites (UV) avec s.visible_in_stock = true ; filtre s.customer_id = {{CUSTOMER_ID}}. C'est le stock WMS : nomme TOUJOURS l'alias 'stock_wms_uv' (pas juste 'stock') pour lever l'ambiguite. Par reference produit : JOIN logistic_management.master_items mi ON mi.id = s.master_item_id (reference = mi.item_reference). Par entrepot : JOIN analytics.warehouses w ON w.id = s.warehouse_id (nom = w.name). N'utilise PAS master_items_in_stock. Les notions 'reel' et 'previsionnel' ne sont PAS disponibles : precise-le dans intent si demandees.\n" +
    (extra ? extra + "\n" : "") +
