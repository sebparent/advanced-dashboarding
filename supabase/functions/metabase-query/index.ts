...
async function interpret(prompt, tables, extra) {
  ...
  const system = "Tu traduis une demande en francais en une requete SQL de LECTURE SEULE pour un entrepot interroge via Metabase (Postgres).\n" +
    ...
    "   Si l'utilisateur PRECISE la notion (WMS, reel, previsionnel), renvoie UNIQUEMENT celle-la. S'il demande 'le stock' / 'stock total' SANS preciser, renvoie les TROIS en une seule ligne en NE parcourant stock_deltas QU'UNE SEULE FOIS (obligatoire pour la perf) : SELECT (SELECT SUM(quantity) FROM logistic_management.stocks WHERE customer_id = {{CUSTOMER_ID}} AND visible_in_stock) AS stock_wms_uv, SUM(sd.quantity) FILTER (WHERE sd.is_forecasted = false) AS stock_reel_uv, SUM(sd.quantity) AS stock_previsionnel_uv FROM logistic_management.stock_deltas sd JOIN logistic_management.stocks s ON s.id_int = sd.stock_int_id WHERE s.customer_id = {{CUSTOMER_ID}} ; chart_type = table. Par reference : JOIN logistic_management.master_items mi ON mi.id = s.master_item_id (mi.item_reference). Par entrepot : JOIN analytics.warehouses w ON w.id = s.warehouse_id (w.name). N'utilise PAS master_items_in_stock.\n" +
    (extra ? extra + "\n" : "") +
